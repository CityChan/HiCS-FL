...
def get_gradients_average(previous_global_m, global_m, batch, local_models, Batchs):
    """return the `representative gradient` formed by the difference between
    the local work and the sent global model"""
    ...
    previous_global_model_params = [
        tens.detach().cpu().numpy() for tens in list(previous_global_m.parameters())
    ]
    
    
    local_model_grads = []
    
    true_model_grad = [ true_weights - global_weights
                            for true_weights, global_weights in zip(global_model_params, previous_global_model_params)
                       ]
    ...
    for local_params in local_model_params:
        local_model_grads += [
            [
                local_weights - global_weights
                for local_weights, global_weights in zip(
                    local_params, previous_global_model_params
                )
            ]
        ]
    ...
    diff_model_grads = []  
    idx = 0
    for local_model_grad in local_model_grads:
        diff_model_grads += [
            [
                true_grads/batch - local_grads/Batchs[idx]
                for local_grads, true_grads in zip(
                    local_model_grad, true_model_grad
                )
            ]
        ]
        idx += 1
...
def get_matrix_similarity_from_grads_magnitudes(local_model_grads, magnitudes,distance_type):
    """return the similarity matrix where the distance chosen to
    compare two clients is set with `distance_type`"""
    ...
    for i, j in product(range(n_clients), range(n_clients)):
        metric_matrix[i, j] = get_similarity(
            local_model_grads[i], local_model_grads[j], distance_type
        ) + np.exp(-100*abs(np.sum(magnitudes[i]) - np.sum(magnitudes[j]))) 
